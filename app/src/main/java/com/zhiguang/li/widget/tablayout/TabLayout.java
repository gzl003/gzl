...
public class TabLayout extends HorizontalScrollView {
    ...
    public TabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.HxTabLayout,
                defStyleAttr, 0);

        mTabStrip.setSelectedIndicatorDrawable(a.getDrawable(R.styleable.HxTabLayout_tabIndicatorDrawable));

        mTabPaddingStart = mTabPaddingTop = mTabPaddingEnd = mTabPaddingBottom = a
                .getDimensionPixelSize(R.styleable.HxTabLayout_tabPadding, 0);
        mTabPaddingStart = a.getDimensionPixelSize(R.styleable.HxTabLayout_tabPaddingStart,
                mTabPaddingStart);
        mTabPaddingTop = a.getDimensionPixelSize(R.styleable.HxTabLayout_tabPaddingTop,
                mTabPaddingTop);
        mTabPaddingEnd = a.getDimensionPixelSize(R.styleable.HxTabLayout_tabPaddingEnd,
                mTabPaddingEnd);
        mTabPaddingBottom = a.getDimensionPixelSize(R.styleable.HxTabLayout_tabPaddingBottom,
                mTabPaddingBottom);

        mTabTextAppearance = a.getResourceId(R.styleable.HxTabLayout_tabTextAppearance,
        ...
        if (a.hasValue(R.styleable.HxTabLayout_tabTextColor)) {
            // If we have an explicit text color set, use it instead
            mTabTextColors = a.getColorStateList(R.styleable.HxTabLayout_tabTextColor);
        }

        if (a.hasValue(R.styleable.HxTabLayout_tabSelectedTextColor)) {
            // We have an explicit selected text color set, so we need to make merge it with the
            // current colors. This is exposed so that developers can use theme attributes to set
            // this (theme attrs in ColorStateLists are Lollipop+)
            final int selected = a.getColor(R.styleable.HxTabLayout_tabSelectedTextColor, 0);
            mTabTextColors = createColorStateList(mTabTextColors.getDefaultColor(), selected);
        }

        mRequestedTabMinWidth = a.getDimensionPixelSize(R.styleable.HxTabLayout_tabMinWidth,
                INVALID_WIDTH);
        mRequestedTabMaxWidth = a.getDimensionPixelSize(R.styleable.HxTabLayout_tabMaxWidth,
                INVALID_WIDTH);
     ...
     *
     * @param drawable drawable to use for the indicator
...
//                            .inflate(R.layout.design_layout_tab_text, this, false);
    ...
    public class SlidingTabStrip extends LinearLayout {
        ...
        private float mSelectedIndicatorMinWhite;
...
//            indicatorDrawable = ContextCompat.getDrawable(context, R.drawable.indicator_red);
//            indicatorDrawable = ContextCompat.getDrawable(context,R.drawable.ic_hx_red_icon);
            if (indicatorDrawable != null) {
                mSelectedIndicatorMinWhite = indicatorDrawable.getIntrinsicWidth();
                mSelectedIndicatorHeight = indicatorDrawable.getIntrinsicHeight();
            }
        }
        ...
        public void draw(Canvas canvas) {
            ...
            indicatorDrawable.setBounds((int) left, getHeight() - mSelectedIndicatorHeight, (int) right, getHeight());
    ...
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        ...
        public void onPageScrolled(final int position, final float positionOffset,
            ...
            // Indicator 的宽度
            float stripWhite = tabLayout.mTabStrip.mSelectedIndicatorMinWhite;
            // 左滑（右侧页面进入）的第一阶段
            // 以及右滑（左侧页面进入）的第二阶段
            if (positionOffset > 0 && positionOffset < 0.5) {
                tabLayout.mTabStrip.left = tabLayout.mTabStrip.getChildAt(position).getLeft()
                        + (tabLayout.mTabStrip.getChildAt(position).getWidth() >> 1) - stripWhite / 2;

                //左边下标的Right
                float lr = tabLayout.mTabStrip.getChildAt(position).getLeft() +
                        (tabLayout.mTabStrip.getChildAt(position).getWidth() >> 1) + stripWhite / 2;
                //右边下标的Right
                float rr = tabLayout.mTabStrip.getChildAt(position + 1).getLeft() +
                        (tabLayout.mTabStrip.getChildAt(position + 1).getWidth() >> 1) + stripWhite / 2;
                //根据滑动偏移更新下标 right
                tabLayout.mTabStrip.right = lr + (positionOffset * 2) * (rr - lr);
            ...
            if (positionOffset >= 0.5) {
                //右边下标的Right
                tabLayout.mTabStrip.right = tabLayout.mTabStrip.getChildAt(position + 1).getLeft() +
                        (tabLayout.mTabStrip.getChildAt(position + 1).getWidth() >> 1) + stripWhite / 2;
                ...
                float rl = tabLayout.mTabStrip.getChildAt(position + 1).getLeft()
                        + (tabLayout.mTabStrip.getChildAt(position + 1).getWidth() >> 1) - stripWhite / 2;
                tabLayout.mTabStrip.left = ll + ((positionOffset - 0.5f) * 2) * (rl - ll);
                ViewCompat.postInvalidateOnAnimation(tabLayout.mTabStrip);
            }
//            // 滑动开始或结束
            if (positionOffset == 0) {
                tabLayout.mTabStrip.left = tabLayout.mTabStrip.getChildAt(position).getLeft()
                        + (tabLayout.mTabStrip.getChildAt(position).getWidth() >> 1) - stripWhite / 2;
                tabLayout.mTabStrip.right = tabLayout.mTabStrip.getChildAt(position).getLeft()
                        + (tabLayout.mTabStrip.getChildAt(position).getWidth() >> 1) + stripWhite / 2;
                ViewCompat.postInvalidateOnAnimation(tabLayout.mTabStrip);
            }
            Log.d(TAG, "positionOffset: " + positionOffset);
            Log.d(TAG, "mTabStrip.left: " + tabLayout.mTabStrip.left + "/   mTabStrip.right: " + tabLayout.mTabStrip.right);
        }
